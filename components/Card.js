    ...
    return (
        <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
            <div className="">
                ...
                <img className="h-48 w-full object-cover md:h-full md:w-full" src={imgsrc} alt={imgalt}/>
                </div>
                <div className="p-8">
                <div className="uppercase tracking-wide text-md md:text-sm text-indigo-500 font-semibold">{heading}</div>
                <p className="mt-2 text-slate-500 hidden md:hidden lg:inline-flex">{desc}</p>
                </div>
            </div>
        </div>
    );
